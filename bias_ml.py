...
def main():
   ...
   news_corpus = pd.read_csv('../data/articles.csv')
   ...
   X = X.drop(['political_bias', 'url'], axis=1)

   logging.info(X)

   logging.info("Preforming get_dummie on X")
   X = pd.get_dummies(X, dtype=float, columns=['topic_x', 'source_name', 'biasness'])
   ...
   logging.info("creating train_prefictions from clf.predict(x_best)")
